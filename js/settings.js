...
const connectBtn = document.getElementsByClassName("connectBtn");
const connectedBtn = document.getElementsByClassName("connectedBtn");

const eyeOff = document.getElementsByClassName("eyeOff");
...
const connectBtnArray = Array.from(connectBtn);
const connectedBtnArray = Array.from(connectedBtn);

connectBtnArray.forEach((btn, index) => {
    btn.addEventListener('click', () => {
        showHide(connectedBtn[index]);
        connectBtn[index].style.display = 'none'
    });
});
connectedBtnArray.forEach((btn, index) => {
    btn.addEventListener('click', () => {
        showHide(connectBtn[index]);
        connectedBtn[index].style.display = 'none'
    });
});
